import {
  ...
  export default function SeriesPlay() {
      ...
      if (params && params.id) {
        try {
          const responseSeries = await axios.get(
            `http://localhost:3080/episode1/${params.series_id}/${params.episode}`
          );
    ...
    const fetchEpisodeSeries = async () => {
      if (params && params.series_id && params.episode) {
        try {
    ...
    console.log(params.series_id,"aab")
    console.log(params.episode,"aac")
    console.log(dataSeries,"juj")
    useEffect(() => {
      fetchDataSeries();
      fetchDescSeries();
      fetchEpisodeSeries();
